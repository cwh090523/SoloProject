...
namespace UI.Presenter
{
    public class TitlePresenter : MonoBehaviour
    {
        ...
        private int FindResolutionIndex(int width, int height)
        {
            ...
            return FindResolutionIndex(Screen.currentResolution.width, Screen.currentResolution.height, false);
        }

        private int FindResolutionIndex(int width, int height, bool allowFallback)
        {
            for (int i = 0; i < _resolutionOptions.Count; i++)
            {
                if (_resolutionOptions[i].width == width && _resolutionOptions[i].height == height)
            ...
            return allowFallback ? 0 : Mathf.Max(0, _resolutionOptions.Count - 1);
        }
        ...
        private void RegisterHover(Button button)
        {
            ...
            button.RegisterCallback<MouseEnterEvent>(HandleButtonMouseEnter);
            button.RegisterCallback<MouseLeaveEvent>(HandleButtonMouseLeave);
        }

        private void UnregisterHover(Button button)
        {
            if (button == null)
                return;

            button.UnregisterCallback<MouseEnterEvent>(HandleButtonMouseEnter);
            button.UnregisterCallback<MouseLeaveEvent>(HandleButtonMouseLeave);
        }

        private void HandleButtonMouseEnter(MouseEnterEvent evt)
        {
            if (evt.currentTarget is VisualElement element)
                SetButtonHighlight(element, true);
        }

        private void HandleButtonMouseLeave(MouseLeaveEvent evt)
        {
            if (evt.currentTarget is VisualElement element)
                SetButtonHighlight(element, false);
        }

        private void SetButtonHighlight(VisualElement element, bool isHighlighted)
        {
            float borderWidth = isHighlighted ? 3f : 0f;
            Color borderColor = isHighlighted ? HoverBorderColor : NormalBorderColor;
